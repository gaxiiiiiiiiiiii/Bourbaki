...
lemma Subtopology.isOpen_eq {X : Type _} [HX : Topology X] (A : Set X) :
...
:= by classical
  ...
  have EA : (fun _ => HA) = HA' := by simp [HA', HA]; ext _ U; rw[Subtopology.isOpen_eq]
...
:= by
  intro B
  constructor<;> intro H
  · intro i; rw [Subtopology.isOpen_eq]; use B
  · have E : B = ⋃ i, B ∩ interior (A i) := by {
    ...
    have Hi := H i; rw [Subtopology.isOpen_eq] at Hi
...
:= by
  ...
  · intro i
    ...
    rw [Subtopology.isOpen_eq]; use Bᶜ, H
    ext x; simp
  ·
    have E : B = ⋃ i, B ∩  (A i) := by {
      ext x; simp; intro Bx
